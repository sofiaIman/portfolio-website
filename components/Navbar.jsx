...
const Navbar = () => {
  ...
  return (
    <div className="w-full h-20 z-10 fixed bg-zinc-900 text-gray-200 duration-300 ease-in">
      <div className="flex justify-between items-center w-full h-full max-w-screen-xl mx-auto p-4">
        <Link href="/#home">
          <h1 className="text-3xl lg:text-4xl font-bold capitalize   tracking-wider cursor-pointer ">
            Sofia
          </h1>
        </Link>
        <div>
      ...
      >
        <div
          className={
            navigation
              ? "fixed left-0 top-0 w-4/5 h-full bg-gradient-to-r from-blue-900 to-cyan-800 text-white p-10 ease-in duration-500"
              ...
              {links.map(({ id, link }) => (
                <Link key={id} href={`/#${link}`}>
                  <li onClick={() => setNavigation(false)}
                   className="py-4 text-2xl tracking-wider cursor-pointer">{link}</li>
                </Link>
              ))}
            </ul>
